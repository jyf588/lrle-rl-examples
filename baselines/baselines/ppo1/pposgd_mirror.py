    ...
    while True:
        ...
        if new:
            ...
            if not broke:
                ...
                ep_rets.append(cur_ep_ret - rew)
                ep_lens.append(cur_ep_len - 1)
                t -= 1
            cur_ep_ret = 0
            cur_ep_len = 0
            ob = env.reset()
...
def learn(env, policy_fn, *,
    ...
    if policy_scope is None:
        ...
        pi = policy_fn(policy_scope, ob_space, ac_space)  # Construct network for new policy
        oldpi = policy_fn("old" + policy_scope, ob_space, ac_space)  # Network for old policy
    ...
    while True:
        ...
        lrlocal = (seg["ep_lens"], seg["ep_rets"], seg["avg_vels"])  # local values
        listoflrpairs = MPI.COMM_WORLD.allgather(lrlocal)  # list of tuples
        lens, rews, avg_vels = map(flatten_lists, zip(*listoflrpairs))
        ...
        rollout_length_threshold_satisfied = True
        if rollout_length_threshold is not None:
            ...
            if not (np.mean(lenbuffer) > rollout_length_threshold and np.mean(avg_vels) > 0.5 * env.env.env.final_tv):
                rollout_length_threshold_satisfied = False
        if rollout_length_threshold is not None or return_threshold is not None:
            if rollout_length_threshold_satisfied and return_threshold_satisfied:
                break
